...
class SetCoordsParamsInGlobal extends IAction {
  ...
  void action(IExtractable? arg) async {
    SignalStrength x = ContainerExtractor.extract<SignalStrength>(
        constantContainer, 'SignalParamsExtractor');
    ...
    ContainerExtender.extend(
        valueContainer, 'cellSignalStrength', gcss ?? 'Waiting');
    ContainerExtender.extend(
        valueContainer, 'wifiSignalStrength', gwfss ?? 'Waiting');
  }
}
